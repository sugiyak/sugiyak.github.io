...
const area = {
  ...
  function changeDirectionIfNecessary(ball, x, y) {
    ...
    if (y < area.height/2 || y > area.height - ball.height) {
      ball.dy = -ball.dy;
    }
  }
  ...
  initialize();
  const ball1 = create('blue', 3, 1);
  const ball2 = create('red', 1, 1);
  const ball3 = create('green', 2, 1);
  moveTo(ball1, 10, 10);
  moveTo(ball2, 10, 10);
  moveTo(ball3, 10, 10);
  ...
  // This is expected to make the 3 balls move around the area div
   ...
   update(ball1, 250, 250);
   update(ball2, 250, 250);
   update(ball3, 250, 250);
